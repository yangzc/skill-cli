# skm - namespace-based skill manager

`skm` 用**命名空间**管理 skill。**一个命名空间 = 一个 git 仓库**；skill 就是仓库里含 `SKILL.md` 的目录。
装好后 `skm` 是个普通命令，可以在**任意文件夹**下使用。

```bash
skm ns add heygen-com/hyperframes   # 建命名空间（自动取名 + 自动切换为当前）
skm ls -v                           # 看这个仓库里有哪些 skill
skm install hyperframes-core media-use  # 装几个
skm ns use vercel-labs/skills ...   # 切到另一个命名空间
skm update --installed              # 拉取上游更新并重装
```

## 安装

```bash
ln -sf "$PWD/bin/skm.js" ~/.local/bin/skm   # ~/.local/bin 已在 PATH 中
skm version
```

（也可以 `npm link`，或直接 `node bin/skm.js ...`。要求 Node ≥ 18、系统有 `git`，无第三方 npm 依赖。）

## 核心概念

| 概念 | 说明 |
|---|---|
| **命名空间** | 一个 git 仓库。名字默认从仓库名推导（`heygen-com/hyperframes` → `hyperframes`） |
| **当前命名空间** | 全局存一份，`skm ls` / `install` / `rm` / `update` 默认作用于它 |
| **仓库缓存** | 每个命名空间一份工作副本：`~/.cache/skm/repos/<名字>/` |
| **目标目录（scope）** | skill 实际安装到哪：项目级 `./<skill-name>/`（当前工作目录，每 skill 一目录）、全局 `~/.agents/skills`，或 `--dir` |
...
| 命令 | 说明 |
|---|---|
| `skm ns add <url>` | 新建（名字从仓库推导）；第一个命名空间自动成为当前 |
| `skm ns add <name> <url>` | 指定名字新建 |
...
| 你写的 | 归一化为 | 命名空间名 |
|---|---|---|
| `heygen-com/hyperframes` | `https://github.com/heygen-com/hyperframes.git` | `hyperframes` |
| `heygen-com/hyperframes.git` | 同上 | `hyperframes` |
...
- **`owner/repo` 只展开为 GitHub**。GitLab / 自建 Git 必须写完整 URL（`gitlab.com/a/b` 会直接报错，不会猜错）
- 名字重名/不喜欢时用显式形式：`skm ns add <name> <url>`，名字只允许字母、数字、`.`、`_`、`-`
...
| 命令 | 说明 |
|---|---|
| `skm ls` | 查：列出该命名空间所有 skill，标记哪些已装（`-v` 显示描述） |
| `skm ls --installed` | 列出本目标目录里已安装的，带命名空间 / commit / 时间 |
| `skm ls --all-ns` | 跨所有命名空间列出 |
| `skm install <skill...>` | 增：安装（支持前缀模糊匹配，如 `skm install media`） |
| `skm install --all` | 装该命名空间全部 skill |
| `skm rm <skill...>` | 删：卸载（`--verify` 检查本地改动） |
| `skm rm --all` | 卸载当前命名空间的全部 |
| `skm rm --all-ns` | 卸载本目标目录里的全部 |
| `skm update` | 获取：`git fetch` 并报告上游变化（新增 / 删除 / 改动） |
...
三种等价写法（目标不是当前命名空间时用后两种）：

```bash
skm ns use hyperframes && skm install media-use   # 先切过去
skm install hyperframes:media-use                 # ns:skill 内联
skm install media-use -n hyperframes              # -n / --ns / --namespace
```

- 名字支持**唯一前缀模糊匹配**：`skm install media` 命中 `media-use` 就直接装；命中多个会列出候选并报错
- 也可以给 repo 内相对路径：`skm install hyperframes:skills/media-use`
- `--name <别名>` 装成另一个目录名（仅限单个 skill）：`skm install media-use -n hyperframes --name mua`
...
- **默认 commit + push**。名字叫 push 就真的会推；想先看 diff 再推就用 `--local`，之后直接再跑一次不带 `--local` 的 `skm push <skill>` 即可（空改动时会只推送积压的提交）
...
`skm` 完全走系统 git，所以私有仓库用你的 ssh key。

**`skm push` 默认只问一次口令。** 一次 push 有两次网络操作（先 fetch 再 push），本来会问两次；`skm` 会给 git 注入一个 ssh 连接复用命令，两条操作走同一条连接，而且这条连接在进程退出后还保留 10 分钟：
...
| `--dir <path>` | 指定目标目录 |
| `-f, --force` | 覆盖已存在 |
| `--as <name>` | 安装时重命名（单个 skill） |
| `--refresh` | 同默认行为：安装前先 `git fetch` 最新 |
...
└── src/
    ├── commands.js   # 命令解析与全部子命令（ns / ls / install / rm / update / info）
